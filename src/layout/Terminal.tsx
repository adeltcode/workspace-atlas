...
export default function Terminal() {
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      addLine(exitCode === 0 ? '  exited 0' : `  exited ${exitCode}`, exitCode === 0 ? 'success' : 'error')
    } catch (e) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      e.preventDefault()
      setHistIdx(prev => { const next = Math.min(prev + 1, history.length - 1); if (history[next] !== undefined) setInput(history[next]); return next })
    } else if (e.key === 'ArrowDown') {
      e.preventDefault()
      setHistIdx(prev => { const next = Math.max(prev - 1, -1); setInput(next === -1 ? '' : (history[next] ?? '')); return next })
    }
  }
    ...
    >
      ...
      <div className="terminal-header" onClick={toggleTerminal}>
        <div className="terminal-header-left">
          ...
          {terminalLines.length > 0 && <span className="terminal-line-count">{terminalLines.length}</span>}
        ...
        <div className="terminal-header-right" onClick={e => e.stopPropagation()}>
          ...
          <button className="terminal-btn" onClick={toggleTerminal} title={terminalOpen ? 'Collapse' : 'Expand'}>
      ...
      {terminalOpen && (
        <>
          <div className="terminal-body" ref={bodyRef}>
            {terminalLines.length === 0
              ? <span className="terminal-empty">Type a command below, or run a Docker operation to see output here.</span>
              : terminalLines.map(line => (
                  <div key={line.id} className={clsx('terminal-line', `tl-${line.type}`)}>
                    <span className="tl-ts">
                      {new Date(line.ts).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })}
                    </span>
